...
const categories = () => {
  ...
  const { data, isPending, error } = useFetch('http://localhost:8087/api/categories');
  ...
  return (
    <div className="m-2 md:m-10 mt-24 p-2 md:p-10 rounded-3xl text-white bg-gray-800 dark:text-gray-800">
      ...
      { books && (
        <DataTable headers={headers} nameItem="category" data={filtredBooks} url="http://localhost:8087/api/categories" isEditModalOpen={isEditModalOpen} setIsEditModalOpen={setIsEditModalOpen} isNewUser={isNewBook} setIsNewUser={setIsNewBook} setIsChanged={setIsChanged} />
      )}
    </div>
  );
};
